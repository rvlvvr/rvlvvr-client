...
var keybaseUtil = require('node-keybase/util')
...
var GPGMessageRegexp = /^-----BEGIN PGP MESSAGE-----Version: GnuPG [A-Za-z0-9\/\+=]*-----END PGP MESSAGE-----$/;
var newlineRegexp = /[\r\n]/g;
...
var me = nconf.get('me');
var passphrase = nconf.get('passphrase');
var keybase = new Keybase(me, passphrase)
var publicKey = null
var privateKey = null
var users = [];
...
keybase.login(function(err, result) {
  ...
  publicKey = openpgp.key.readArmored(result.me.public_keys.primary.bundle).keys

  keybaseUtil.import_from_p3skb({raw: result.me.private_keys.primary.bundle, passphrase: passphrase}, function(err, private_key_data) {
    if (err) {
      throw err;
      return;
    }

    privateKey = openpgp.key.readArmored(private_key_data).keys[0];
...
exports.decrypt = function (request, reply) {
  ...
  if (data.sender !== me) {
    ...
    var message = openpgp.message.readArmored(data.text)
    openpgp.decryptMessage(privateKey, message).then(function(decryptedMessage) {
      webremix.generate(decryptedMessage, function (err, fmsg) {
        data.html = fmsg;
