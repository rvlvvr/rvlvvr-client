  ...
  setDatabase(keyName, function (err, k) {
    if (!err) {
      dual[keyName].getChats(true, '\xff', function (err, c) {
        ...
        c.chats.reverse();
        socket.emit('localall', c.chats);

        var lastIdx = 0;

        if (c.chats.length > 0) {
          lastIdx = c.chats.length - 1;
          socket.emit('latest-message-id', c.chats[lastIdx].value.message.created);
        }
      });
    }
  });
...
exports.decrypt = function (data, localSocket) {
  ...
  var saveMessage = function (text) {
    webremix.generate(text, function (err, fmsg) {
      data.html = fmsg;
      console.log('saving remote message ', data);
...
exports.addMessage = function (payload, localSocket) {
  ...
  var saveMessage = function (messageObj, key, original) {
    ...
    webremix.generate(original || localObj.text.toString('utf8'), function (err, result) {
      if (!err) {
