...
var isGPGMessage = exports.isGPGMessage = function (message) {
  return !!message.replace(newlineRegexp, '').match(GPGMessageRegexp);
};

exports.encrypt = function (messageObj, message, next) {
  var args = ['--encrypt', '--armor', '--no-version',
              '--batch', '--recipient', messageObj.pubKey];

  if (nconf.get('alwaysTrust')) {
    args.push('--trust-model=always');
  }

  if (messageObj.encryptingUser) {
    args.push('-u', messageObj.encryptingUser);
  }

  var proc = spawn('gpg2', args);
  var stdout = '';
  var buffers = [];
  var buffLength = 0;
  var error = '';
...
exports.decrypt = function (data, next) {
  ...
  var proc = spawn('gpg2', ['--use-agent', '--decrypt'], { stdin: 'pipe' });
  var stdout = '';
  var error = '';
